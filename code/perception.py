...
def find_rocks(img, levels=(110, 110, 50)):
    ...
    rockcolor = ((img[:,:,0] > levels[0])\
                & (img[:,:,1] > levels[1])\
                & (img[:,:,2] < levels[2]))

    #color_select = np.zeros_like(img[:,:,0])
    color_select[rockcolor] = 1
...
def perception_step(Rover):
    ...
    col_thresh = color_thresh(warped)
    obs_map = np.absolute(np.float32(col_thresh)-1) * mask
    # 4) Update Rover.vision_image (this will be displayed on left side of screen)
        # Example: Rover.vision_image[:,:,0] = obstacle color-thresholded binary image
        #          Rover.vision_image[:,:,1] = rock_sample color-thresholded binary image
        #          Rover.vision_image[:,:,2] = navigable terrain color-thresholded binary image
    Rover.vision_image[:,:,2] = col_thresh * 255
    Rover.vision_image[:,:,0] = obs_map * 255

    # 5) Convert map image pixel values to rover-centric coords
    # Extract navigable terrain pixels
    xpix, ypix = rover_coords(col_thresh)
    ...
    Rover.worldmap[obsy_world, obs_x_world, 0] += 1
